DMR_downsampling_bg_matching
================
KatrínM
2025-11-07

# Generating Background BED files

First, I need a subset bed file, that contains the top 30K most
differentially methylated Dnmt1-DMRs.  
Then, I need background genomic regions that do not contain the
DNmt1-DMRs, but are of similar length and density as the top 30K DMRs.

## A function that will downsample the DMR list based on absolute_descending dIF values (column 8 in the DMR file)
...
Now I need to generate a background in the mm10 genome that is size and
CpG-density matched to these top 30K DMRs, does not include duplicated
or overlapping regions and is not found in the full DMR list \#
————————————————————————-
