...
BSseq.obj <- readRDS("BSseq.obj.smoothed.no.any.filtering.rds")
...
keepLoci.ex <- which(rowSums(cov[, BSseq.obj$Groups == "EM_gene"] >= 3) >= 45 &
                       rowSums(cov[, BSseq.obj$Groups == "Control"] >= 3) >= 13)
...
group1 = rownames(pData(BSseq.obj)[pData(BSseq.obj)$Groups == 'EM_gene',])
...
t.test <- BSmooth.tstat(BSseq.obj,
                               ...
                               estimate.var = "same",
                               local.correct = TRUE,
                               verbose = TRUE, k = 21, mc.cores = 10, maxGap = 10^8)
...
smoothed_methylation <- getMeth(BSseq.obj, putative.DMRs.with.FWER, type = "smooth", what = "perRegion")
write.csv(smoothed_methylation, "smoothed_methylation_of_putative_DMRs.csv", quote = F, row.names = F)

# Draw heatmap of all DMRs based on smoothed methylation values

pdf("heatmap.of.smoothed.putative.DMRs.pdf")
