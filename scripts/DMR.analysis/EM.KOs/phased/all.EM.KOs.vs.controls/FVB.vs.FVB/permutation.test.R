...
subsetDmrs <- function(nullDist) {
  ...
  for (i in 1:length(nullDist)) {
    ...
    out <- xx[xx[, "n"] >= 3 & abs(xx[, "meanDiff"]) >= 0.015 &
                xx[, "invdensity"] <= 300, ]
...
keepLoci.ex <- which(rowSums(cov[, BSseq.obj$Groups == "EM.gene"] >= 3) >= 50 &
                       rowSums(cov[, BSseq.obj$Groups == "Control"] >= 3) >= 14)
...
subset.putative.DMRs <- subset(Putative.DMRs, n >= 3 & abs(meanDiff) >= 0.015 & invdensity <= 300)
