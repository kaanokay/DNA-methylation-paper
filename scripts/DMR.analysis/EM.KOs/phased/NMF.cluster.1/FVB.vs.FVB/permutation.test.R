...
# Keep only FVB samples

BSseq.obj <- BSseq.obj[, grepl(".FVB.", pData(BSseq.obj)$Samples)]

# Select NMF cluster 1 EM genes: Kdm5c, Kdm5b, Kdm1a, Dnmt3b, Chd5
# Note: Dnmt3b is poor qualit sample, so I removed it from this cluster!

selected_samples <- c("Chd3", "Setd4", "Set1a", "Set1b", "Kdm5b", "Kdm1a", "Ctr")

BSseq.obj <- BSseq.obj[, grepl(paste(selected_samples, collapse = "|"), pData(BSseq.obj)$Samples)]
...
keepLoci.ex <- which(rowSums(cov[, BSseq.obj$Groups == "EM.gene"] >= 2) >= 6 &
                       rowSums(cov[, BSseq.obj$Groups == "Control"] >= 2) >= 14)
...
pData(BSseq.obj)$col <- metadata$Color
...
t.test <- BSmooth.tstat(BSseq.obj,
                               ...
                               estimate.var = "same",
                               local.correct = TRUE,
                               verbose = TRUE, k = 21, mc.cores = 40, maxGap = 10^8)
...
pdf(file = "first.200.DMRs.pdf", width = 10, height = 5)
